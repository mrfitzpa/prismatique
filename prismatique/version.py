...
def _get_git_revision():
    """Get revision hash of ``prismatique`` from git.
    ...
    -------
    revision : `str`
        Git revision hash of ``prismatique``.
    """
    try:
...
def _get_full_version():
    """Get version of ``prismatique`` from git.
...
def _get_version_summary():
    """Get version summary of ``prismatique``.
    ...
    -------
    summary : `str`
        Version summary of ``prismatique``.
    """
    # Check versions of prismatique.
    from . import _version
    if _version.version != version:
        raise ValueError("prismatique version has changed since "
                         "installation/compilation")



    # Generate summary.
    summary = ("prismatique {prismatique_ver!s};\n"
               "git revision {git_rev!s} using\n"
               "python {python_ver!s}")
    
    summary = summary.format(prismatique_ver=full_version,
